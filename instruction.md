...
The runner at `/app/runner.js` still contains a `loadRoomFromFile()` function that opens files from `/app/legacy_rooms/*.txt` directly. Your task is to remove all uses of this function and replace them with calls to the local API so that the runner no longer opens any files under `legacy_rooms/` during play.
